...
$(function() {
	...
	$("#dg").datagrid({
		...
		queryParams: {
			defects_check : 0,
			defects_profitLoss : 0 ,
			defects_purchasedCheck : 1,
		},
		columns :[ [{
			field : 'defects_date',
			title : '日期',
...
function purchasedCheck(){
	...
	if(nodes.length==0){
		util.show("请选择采购部审核条目");
		return;
	}
	var data="";
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].defects_purchasedOpinion ==""){
			util.show("请输入采购部审核意见");
			return;
		}
		//将行数据封装成字符串，以便传到后台
		data+=nodes[i].defects_id+"_";
		data+=nodes[i].defects_purchasedOpinion;
		
		if(i<nodes.length-1)
			data+="|";
	}
	var re = util.get(global_param.context_name + "/quality/purchased/purchasedCheck?data="+data);
...
function deleteFunc() {
	...
	util.confirm("确定要执行删除操作？", function() {
		var data = util.del(global_param.context_name + "/quality/purchased/" + node.defects_id);
