...
$(function() {
	...
	$("#dg").datagrid({
		...
		columns :[ [{
			...
			field : 'defects_financialdPerson',
			title : '财务部审核人',
			width : 80,
			sortable : true
		}, {
			...
			field : 'defects_managerPerson',
			title : '经理审核人',
			width : 80,
			sortable : true
		}, {
			field : 'defects_managerOpinion',
			title : '经理意见',
			width : 150,
		}, {
			field : 'defects_managerTime',
			title : '经理审核时间',
			width : 100,
			sortable : true,
			formatter : function (value, row, index) {
		...
		onLoadSuccess:function(data){
			var defects_breakageTicketNo = util.get(global_param.context_name + "/quality/breakage/getMaxDefectsBreakageTicket");
			$("#defects_breakageTicketNo").val(defects_breakageTicketNo);
...
function breakageFunc(){
	...
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].defects_breakageReason ==""){
			util.show("请输入损益原因");
			return;
		}
		//将行数据封装成字符串，以便传到后台
		data+=nodes[i].defects_id+"_";
		data+=nodes[i].defects_breakageReason;
		
		if(i<nodes.length-1)
			data+="|";
	}
	/*var re = util.get(global_param.context_name + "/quality/breakage/breakageCheck?data="+data);
	...
	//报损确认操作
	var result = util.get("/HRERP/quality/breakage/breakageCheck?data="+data+
			"&oldBreakageTicket="+$("#defects_breakageTicketNo").val());
...
function deleteFunc() {
	...
	util.confirm("确定要执行删除操作？", function() {
		var data = util.del(global_param.context_name + "/quality/breakage/" + node.defects_id);
