...
function check(){
	...
	if(nodes.length==0){
		util.show("请选择不合格品种信息");
		return;
	}
	var data="";
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].defects_checkSuggestion ==""){
			util.show("请输入确认意见");
			return;
		}
		//将行数据封装成字符串，以便传到后台
		data+=nodes[i].defects_id+"_";
		data+=nodes[i].defects_checkSuggestion;
		
		if(i<nodes.length-1)
			data+="|";
	}
	var re = util.get(global_param.context_name + "/quality/confirm/check?data="+data);
...
function deleteFunc() {
	...
	util.confirm("确定要执行删除操作？", function() {
		var data = util.del(global_param.context_name + "/quality/confirm/" + node.defects_id);
