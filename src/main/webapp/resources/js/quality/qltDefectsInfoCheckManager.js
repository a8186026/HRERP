...
$(function() {
	...
	$("#dg").datagrid({
		...
		queryParams: {
			defects_financialdCheck : 0,
			defects_managerCheck : 1,
		},
		columns :[ [{
			field : 'defects_date',
			title : '日期',
...
function managerCheck(){
	...
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].defects_managerOpinion ==""){
			util.show("请输入经理审核意见");
			return;
		}
		//将行数据封装成字符串，以便传到后台
		data+=nodes[i].defects_id+"_";
		data+=nodes[i].defects_managerOpinion;
		
		if(i<nodes.length-1)
			data+="|";
	}
	var re = util.get(global_param.context_name + "/quality/manager/managerCheck?data="+data);
...
function deleteFunc() {
	...
	util.confirm("确定要执行删除操作？", function() {
		var data = util.del(global_param.context_name + "/quality/manager/" + node.defects_id);
