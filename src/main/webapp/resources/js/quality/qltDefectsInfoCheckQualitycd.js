...
$(function() {
	...
	$("#dg").datagrid({
		...
		queryParams: {
			defects_purchasedCheck : 0,
			defects_qualitycdCheck : 1,
		},
		columns :[ [{
			field : 'defects_date',
			title : '日期',
...
function qualitycdCheck(){
	...
	if(nodes.length==0){
		util.show("请选择质管部审核条目");
		return;
	}
	var data="";
	for(var i=0;i<nodes.length;i++){
		if(nodes[i].defects_qualitycdOpinion ==""){
			util.show("请输入质管部审核意见");
			return;
		}
		//将行数据封装成字符串，以便传到后台
		data+=nodes[i].defects_id+"_";
		data+=nodes[i].defects_qualitycdOpinion;
		
		if(i<nodes.length-1)
			data+="|";
	}
	var re = util.get(global_param.context_name + "/quality/qualitycd/qualitycdCheck?data="+data);
...
function deleteFunc() {
	...
	util.confirm("确定要执行删除操作？", function() {
		var data = util.del(global_param.context_name + "/quality/qualitycd/" + node.defects_id);
