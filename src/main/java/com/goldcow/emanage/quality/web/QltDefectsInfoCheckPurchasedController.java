...
@Controller
@RequestMapping(value = "/quality/purchased")
public class QltDefectsInfoCheckPurchasedController {
	/** 不合格品种信息管理服务 */
	@Autowired
	...
	public String view(HttpServletRequest request) {
		if (SysUtil.hasRight(request, "qltDefectsInfoVO", GlobalVal.MENU_FUNCTION.VIEW)) {
			return "/quality/defectsInfoCheck/qltDefectsInfoCheckPurchasedList.jsp";
		} else {
	 ...
	 * @return 操作结果
	 */
	@RequestMapping(value = "purchasedCheck", method = RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> purchasedCheck(HttpServletRequest request, String data) {
		Map<String, Object> result = Maps.newHashMap();
		request.getParameterMap();
		if (SysUtil.hasRight(request, "qltDefectsInfoVO", GlobalVal.MENU_FUNCTION.UPDATE)) {
			
			qltDefectsInfoService.purchasedCheck(data, request);
