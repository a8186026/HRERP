package com.goldcow.sframe.util.servlet;
...
import com.goldcow.sframe.util.GlobalFileUtil;
...
public class FileHanldeController {
	...
	public String handleFormUpload(HttpServletRequest request,
			HttpSession session) {
		SysUser user = (SysUser) session.getAttribute(GlobalVal.USER_SESSION);
		String result = "";
		if (this.multipartResolver != null
			...
			if (request instanceof MultipartHttpServletRequest) {
				...
				for (Map.Entry<String, MultipartFile> entity : fileMap
					...
					try {
						String path = GlobalFileUtil.saveUploadFile(mf.getInputStream(), ""+user.getUser_id(), mf.getOriginalFilename());
						result = result + path +",";
					} catch (IOException e) {
